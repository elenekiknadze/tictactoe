...
export const SCOREBOARD_SERVICE_TOKEN =
  new InjectionToken<ScoreboardBaseService>('SCOREBOARD SERVICE TOKEN');

@Injectable({
  providedIn: 'root',
})
export abstract class ScoreboardBaseService {
  private scores_: ScoreEntry[] = [];
  //probably need to abstract this out too
  private readonly identifier = 'ticTacToeScores';
  constructor(
    @Inject(NUMBER_OF_SCORES) private readonly numberOfScores: number
  ) {
  ...
  addScore(scoreEntry: ScoreEntry) {
    if (
      !this.scores_.length ||
      scoreEntry.score <= this.scores_[this.scores_.length - 1].score
    ) {
  ...
  abstract calculateScore(...params: any[]): number;
}
